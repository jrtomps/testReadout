...
{
  // this should set m_id
//  readConfig();

  pExperiment->EstablishTrigger(&m_trigger);
//  pExperiment->AddEventSegment(&m_segment);

//  pExperiment->AddEventSegment(new CInserterSegment(pExperiment));

  CGeneratorSegment* pSegment = new CGeneratorSegment(gBufSize, *pExperiment);
  pExperiment->AddEventSegment(pSegment);
  
  
////  CDocumentedPacketManager* docPktMgr = CDocumentedPacketManager::getInstance();
//  CDocumentedPacket *fid = new CDocumentedPacket(1,"firmware","firmware description","0.1");
////  docPktMgr->AddPacket(fid);
}
...
Skeleton::SetupScalers(CExperiment* pExperiment) 
{
  ...
  time.tv_sec = 2;
  time.tv_nsec = 0;
  CTimedTrigger *trig = new CTimedTrigger(time);
  pExperiment->setScalerTrigger(trig);

  CMyScaler* myscaler = new CMyScaler(10);
  pExperiment->AddScalerModule(myscaler);
...
//  CMyBeginCommand* pMyBegin = new CMyBeginCommand(*pInterp);
//  CMyEndCommand* pMyEnd = new CMyEndCommand(*pInterp);
//  pRctl->addCommand(pMyBegin);
...
void Skeleton::readConfig()
{
  std::ifstream file("config.txt");
  // first line should be source id
  std::string descriptor;
  file >> descriptor >> m_id;
